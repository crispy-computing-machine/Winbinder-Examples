...
function save_phpcode($filename)
{
	...
	for($i = 0; $i < $ncontrols; $i++) {
		...
		$phpcode .= $ctrlcode . "_create_control($ctrlparent, {$ct->cclass}, " .
		...
		switch($ct->cclass) {
			...
			case 'ListView':
				if($wb->form[$wb->currentform]->fillheaders)
					$phpcode .= "_set_text(\$ctrl, '$ct->caption');\n";
				break;

			case 'ComboBox':
			case 'ListBox':
				if($wb->form[$wb->currentform]->filllists)
					$phpcode .= "_set_text(\$ctrl, explode(',', '$ct->caption'));\n";
				break;
			...
			case 'Gauge':
				if($wb->form[$wb->currentform]->applyvalues)
					$phpcode .= "_set_value(\$ctrl, $ct->value);\n";
				break;
		}
	} // end control loop
