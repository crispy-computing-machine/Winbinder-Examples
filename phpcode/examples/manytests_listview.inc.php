...
function test_lists($window)
{
	...
	_create_items($list, array(
		array("0", "First", "Done", "Less"),
		array(0, 0, "Second", "Hi"),
		array(200, 300, "Hi!", 876),
	));
	_set_text($list, array(444, 555, 666));
...
//	wb_delete_items($list, 0);
	_set_value($list, array(1,2));
	_set_value($list, array(1,3));
//	wb_set_listview_item_checked($list, 0, 1);

	// ListView II

	$list = wb_get_control($window, IDC_LISTVIEW2067);
	_set_text($list, array("No grid lines","on this control!"));
	_create_items($list, array(
	...
	_set_text(wb_get_control($window, IDC_LISTBOX2018),
		"List 1\r\nOne more\rAnother\nEtc.\r\nMore...");
	_set_text(wb_get_control($window, IDC_COMBOBOX2010),
		array("Option 1","Option 2","More","Etc."));

	_set_value(wb_get_control($window, IDC_UPDOWN2069), null, 0, 3);
...
//	$ctrl = wb_get_control($window, $id);
	...
	switch($id) {
		...
		case IDC_LISTLINE:
			$n = wb_get_value($ctrl);
			$text = _get_text(wb_get_control($window, IDC_LISTVIEW2027), $n);
			if(is_array($text)) {
				$elem = $text[1];
				$text = implode(" ", $text);
			}
			_set_text(wb_get_control($window, IDC_EDIT2064), "$n [$text]\r\n$elem");
			return true;
		...
		case IDC_LISTVIEW2067:
			...
			if($lparam == WBC_DBLCLICK) {
				_set_text(wb_get_control($window, IDC_EDIT2064), "Double-clicked.");
			} else {
				...
				if(@is_array($text[0])) {
					...
					$text = implode(", ", $text);
				_set_text(wb_get_control($window, IDC_EDIT2064), "Selected rows: $sel\nValues: [$val]\nText: $text");
			}
			return true;
		...
		case IDC_COMBOBOX2010:
			$val = wb_get_value($ctrl);
			$text = _get_text($ctrl);
			_set_text(wb_get_control($window, IDC_EDIT2064), "[$text] [val: $val]");
			return true;
	}
	return false;
}
