...
const WBC_SPLIT_VERTICAL = 134217728;
const WBC_SPLIT_HORIZONTAL = 67108864;
...
/**
 * Loads the image file filename from disk and returns a handle to it.
 *
 * index is the index of the image on the file if filename is an icon library.
 *
 ...
 * Roate an image
 *
 * Takes bitmap image handle and the degree's to rotate it and returns the new image handle.
 *
 * @param $hbm
 * @param $angle
 * @return int
 */
function wb_rotate_image(int $hbm, int $angle): int
{
}

/**
 * Resize a bitmap image to the $width and $height and return the new image handle.
 *
 * @param $hbm
 * @param $newWidth
 * @param $newHeight
 * @return int
 */
function wb_resize_image(int $hbm, int $newWidth, int $newHeight): int
{
}
...
/**
 * Refreshes or redraws the WinBinder object wbobject, configured FPS for how often ot refresh.
 *
 * @param int $wbObject
 * @param int|null $fps
 * @param int|null $x
 ...
 * @return bool
 */
function wb_begin_refresh(int $wbObject, ?int $fps = 30, ?int $x = 0, ?int $y = 0, ?int $width = 0, ?int $height = 0) : bool{}
 ...
 * - URL
 * - A file path
 * - A directory path (shows emebeded file explorer)
 * - One of the below special commands:
 *
...
/**
 * Use a proxy for the HTML control.
 *
 * Empy string removes/disables proxy.
 *
 * Also refreshes the control.
 *
 * NOTE: This changes the system wide proxy (ensure firefox is set to not use system proxy settings)
 *       May also effect Docker/WSL network connections
 *
 * @param int $wbObject
 * @param string $proxy
 * @return bool
 */
function wb_set_browser_proxy(int $wbObject, string $proxy){}
...
/**
 * Run queries using WMI
 *
 * e.g.
 * wb_wmi_query('SELECT * FROM Win32_Process')
 *
 * @param string $wquery
 * @return array|bool
 */
function wb_wmi_query(string $wquery) : array|bool{}

/**
 * Get system metric
 * @see: https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getsystemmetrics
 *
 * $wb_fix_theme = function($win){
 * list($w1, $h1) = wb_get_size($win);
 * list($w2, $h2) = wb_get_size($win, true);
 * $ox = wb_get_system_metric(SM_CXFIXEDFRAME); // Get theme offset X
 * $oy = wb_get_system_metric(SM_CYFIXEDFRAME); // Get theme offset Y
 * $tx = (($w1-$w2) - ($ox * 2));
 * $ty = (($h1-$h2) - ($oy * 2));
 * $nw = $w1 + $tx + ($ox * 2);
 * $nh = $h1 + $ty + ($oy * 2);
 * wb_set_size($win, $nw, $nh);
 * return [$tx + $ox, $ty + $oy];
 * };
 *
 * @param int $idx
 * @return int
 */
function wb_get_system_metric(int $idx) : int{}

/**
 * Get Windows timezone
 *
 * $tz = wb_get_system_timezone();
 * date_default_timezone_set($tz);
 * echo date("Y-m-d H:i:s");
 *
 * @return string
 */
function wb_get_system_timezone() : string{}

/**
 * Expand string that uses env variables
 *
 * echo wb_expand_env('%WINDIR%\system32\drivers\etc\hosts');
 *
 * @param string $path
 * @return string|bool
 */
function wb_expand_env(string $path) : string|bool{}

/**
 * Send a key to a window. See VK_* constants
 *
 * wb_send_key(VK_LWIN);
 *
 ...
 * @return bool
 */
function wb_send_key(array|int $keys = 0) : bool{}
 ...
 * @return bool
 */
function wb_is_running(int $pid) : bool{}

/**
 * Get all data about a TTF file.
 *
 * $file = wb_get_system_info('fontpath').'arial.ttf';
 * $info = wb_get_ttf_info($file);
 *
 * @param string $file
 * @return array|bool
 */
function wb_get_ttf_info(string $file) : array|bool{}
 ...
 * @return bool
 */
function wb_set_window_accept_drop(int $pwbo, bool $accept = true) : bool{}

/**
 * Get dropped files array during window main event loop, see WBC_DROPFILES
 *
 * if ($param->param1 == WBC_DROPFILES) {
 *     print_r(wb_get_drop_files($param->param3, true));
 * }
 *
 * @param int $wparam
 * @param bool $isShort
 * @return array|null
 */
function wb_get_drop_files(int $wparam, bool $isShort = false) : ?array{}

/**
 * Bring window to front via handle
 * @param int $pwbo
 * @return bool
 */
function wb_bring_to_front(int $pwbo) : bool{}

/**
 * Get window draw buffer
 * @return int|bool
 */
function wb_get_window_buffer(int $pwbo) : int|bool{}
 ...
 * @return int|bool
 */
function wb_get_window_handle(int $pwbo) : int|bool{}

/**
 * Sets divider position for a Splitter.
 *
 * If ratio_mode is TRUE, position is interpreted as a ratio in the 0..1000 range. Otherwise it is interpreted as pixels from top/left pane start.
 *
 * @param int $wbObject
 * @param int $position
 * @param bool|null $ratio_mode
 * @return bool
 */
function wb_set_splitter_position(int $wbObject, int $position, ?bool $ratio_mode = null) : bool{}
 ...
 * @return int
 */
function wb_get_splitter_position(int $wbObject, ?bool $ratio_mode = null) : int{}

/**
 * Attaches two child controls/windows to a Splitter. The splitter reparents the panes and manages layout automatically.
 *
 * @param int $splitter
 * @param int $pane1
 * @param int $pane2
 * @return bool
 */
function wb_set_splitter_panes(int $splitter, int $pane1, int $pane2) : bool{}

/**
 * Sets minimum pane sizes in pixels for a Splitter.
 *
 * @param int $splitter
 * @param int $min_pane1
 * @param int $min_pane2
